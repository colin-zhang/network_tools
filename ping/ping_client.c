...
static uint64_t send_ok_num = 0;
static uint64_t rev_num = 0;
static uint64_t rev_ok_num = 0;
...
static int icmp_pack(char *send_buf)
{
    ...
    icmp->icmp_cksum = in_cksum((unsigned short *)icmp, packsize, 0);    
    return packsize;
}
...
int ping_recv(struct v4_ping* ptr)
{
    ...
    //Print("checksum = %d\n", icmp->icmp_cksum);
    //icmp->icmp_cksum = 0;
    //Print("checksum = %d\n", in_cksum((unsigned short *)icmp, ip_len - ip_head_len, 0));

    if (ntohs(icmp->icmp_id) == getpid() && 
        icmp->icmp_type == ICMP_ECHOREPLY &&
        icmp->icmp_code == 0 &&
        in_cksum((unsigned short *)icmp, ip_len - ip_head_len, 0) == 0) {
        rev_ok_num++;
...
void test(const char *ip)
{
    ...
    int i, j;
    int t = 1024 * 8;
    struct timespec tm1 = {0, 0};  
    struct timespec tm2 = {0, 0};  

    ptr = ping_addr_init(NULL, 4096);
    if (NULL == ptr) {
...
/*    ping_addr_add(ptr, &ia);
    ...
    ping_addr_add(ptr, &ia);
    get_current_timespec(&tm1);
    for (i = 0; i < t; i++) {
        ping_send(ptr);
        while (fd_poll(ptr->sock_fd, 0)) {
            for (j = 0; j < i; j++) {
                if (ping_recv(ptr) < 0 && errno != 0) {
                   break;
                } 
            }
        }   
    }
    get_current_timespec(&tm2);
    Print("tv_sec = %d, tv_nsec = %d \n", tm2.tv_sec - tm1.tv_sec, tm2.tv_nsec - tm1.tv_nsec);
    ...
    Print("send:%d, %d ok, receive:%d, %d ok\n", send_num, send_ok_num, rev_num, rev_ok_num);

    //ping_show_res(ptr);
    ping_addr_free(ptr);
}

int test2(const char *ip)
{
    struct v4_ping* ptr;
    int ret;
    IA ia;
